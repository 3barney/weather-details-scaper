"""
  class to represent the weather forecast data that the parser returns.
"""
...
class Forecast:
  def __init__(
    self, current_temp, humidity, wind, high_temp=None, low_temp=None, description='',
  ...
  def __str__(self):
    ...
    if (self._forecast_type == ForecastType.TODAY):
      temperature = (f'{offset}{self._current_temp}\xb0\n'f'{offset}High {self._high_temp}\xb0 / 'f'Low {self._low_temp}\xb0 ')
    else:
      temperature = (f'{offset}High {self._high_temp}\xb0 / 'f'Low {self._low_temp}\xb0 ')
